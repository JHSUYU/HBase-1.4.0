...
public class RpcRetryingCaller<T> {
    ...
    public T callWithRetries(RetryingCallable<T> callable, int callTimeout)
        ...
        for (int tries = 0; ; tries++) {
            LOG.debug("Current attempt : " + tries);
//            Exception exception = new Exception();
//            exception.printStackTrace();
            long expectedSleep;
            try {
                LOG.debug("Callable is " + callable.getClass().getName());
                if (callable instanceof ScannerCallableWithReplicas.RetryingRPC) {
                    LOG.info("callable is instance of ScannerCallableWithReplicas.RetryingRPC");
                    if(retryMode){
                        ((ScannerCallableWithReplicas.RetryingRPC) callable).setRetry();
                        retryMode = false;
                        tries--;
                        LOG.info("Enter retry mode ScannerCallableWithReplicas.RetryingRPC");
                    }
                ...
                LOG.debug("Retry Error in RpcRetryingCaller: " + t.getMessage());
        ...
        if (t instanceof ServiceException) {
            ...
            if (cause != null) {
                if (cause instanceof DoNotRetryIOException) {
                    LOG.debug("DoNotRetryIOException from ServiceException", cause);
