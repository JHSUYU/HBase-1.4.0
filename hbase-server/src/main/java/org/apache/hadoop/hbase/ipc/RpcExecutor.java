...
//import org.apache.hadoop.hbase.ipc.Handler;
//
///**
...
//     */
//    protected Handler getHandler(final String name, final double handlerFailureThreshhold, final int handlerCount,
//                                 final BlockingQueue<CallRunner> q, final AtomicInteger activeHandlerCount, final AtomicInteger failedHandlerCount, final Abortable abortable) {
//        return new Handler(name, handlerFailureThreshhold, handlerCount, q, activeHandlerCount, failedHandlerCount,abortable);
//    }
//
...
//                    + ",port=" + port;
//            Handler handler = getHandler(name, handlerFailureThreshhold, handlerCount, callQueues.get(index),
//                    activeHandlerCount, failedHandlerCount, abortable);
//            handler.start();
...
//     */
//
////    class ShadowHandler extends Handler {
////        private CountDownLatch latch;
////
////        public ShadowHandler(String name, double handlerFailureThreshhold, BlockingQueue<CallRunner> q, AtomicInteger activeHandlerCount, boolean isShadow, CountDownLatch latch) {
////            super(name, handlerFailureThreshhold, q, activeHandlerCount, isShadow);
////            this.latch = latch;
////        }
////
////        @Override
...
//                final BlockingQueue<CallRunner> q, final AtomicInteger activeHandlerCount, boolean isShadow) {
//            this(name, handlerFailureThreshhold, q, activeHandlerCount);
...
//                while (running) {
//                    try {
//                        CallRunner curCallRunner = getCallRunner();
//                        Map<Descriptors.FieldDescriptor, Object> fields = curCallRunner.getCall().param.getAllFields();
//                        for (Map.Entry<Descriptors.FieldDescriptor, Object> entry : fields.entrySet()) {
//                            Descriptors.FieldDescriptor field = entry.getKey();
//                            Object value = entry.getValue();
//                            if(field.getName().equals("is_retry") && value.equals(true)){
//                                Cloner cloner = new Cloner();
//                                Handler shadowHandler=cloner.deepClone(this);
//                                shadowHandler.run(curCallRunner);
//                                return;
//                            }
//                        }
...
//                                }
//                            }
//                            LOG.debug("Shadow Handler: callRunner.call.param is " + curCallRunner.getCall().param + " callRunner.call.MethodName is " + curCallRunner.getCall().md.getName() + " callRunner.call.blockingService is" + curCallRunner.getCall().service);
//                            shadowRun(curCallRunner);
//                        } catch (InterruptedException e) {
//                            interrupted = true;
//                        }
//                    }
...
//                    try {
//                        CallRunner curCallRunner = getCallRunner();
//                        LOG.debug("callRunner.call.param is " + curCallRunner.getCall().param + " callRunner.call.MethodName is " + curCallRunner.getCall().md.getName() + " callRunner.call.blockingService is" + curCallRunner.getCall().service);
//                        Map<Descriptors.FieldDescriptor, Object> fields = curCallRunner.getCall().param.getAllFields();
//                        boolean shadowRun = false;
...
//                            if(field.getName().equals("is_retry") && value.equals(true)){
//                                shadowRun = true;
//                                LOG.debug("field is " + field.getName() + " value is " + value);
//                                final BlockingQueue<CallRunner> shadowQueue = new ArrayBlockingQueue<>(1);
//                                shadowQueue.put(curCallRunner);
//                                Handler shadowHandler = new ShadowHandler("shadow", handlerFailureThreshhold, shadowQueue, activeHandlerCount,true, latch);
//                                shadowHandler.start();
//                            }
//                        }
//                        if(shadowRun){
...
//        }
//
//        public void shadowRun(CallRunner cr) {
...
//        }
    ...
    protected void startHandlers(final int port) {
        List<BlockingQueue<CallRunner>> callQueues = getQueues();
        startHandlers(null, handlerCount, callQueues, 0, callQueues.size(), port, activeHandlerCount, handlerCount, failedHandlerCount, abortable);
    }
    ...
    protected void startHandlers(final String nameSuffix, final int numHandlers,
                                 final List<BlockingQueue<CallRunner>> callQueues, final int qindex, final int qsize,
                                 final int port, final AtomicInteger activeHandlerCount, final int handlerCount, final AtomicInteger failedHandlerCount, final Abortable abortable) {
        final String threadPrefix = name + Strings.nullToEmpty(nameSuffix);
        double handlerFailureThreshhold = conf == null ? 1.0 : conf.getDouble(
                HConstants.REGION_SERVER_HANDLER_ABORT_ON_ERROR_PERCENT,
                HConstants.DEFAULT_REGION_SERVER_HANDLER_ABORT_ON_ERROR_PERCENT);
